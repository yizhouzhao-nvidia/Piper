...
import numpy as np
from piper.real.shared_memory.shared_memory_util import ArraySpec, SharedAtomicCounter
from piper.real.shared_memory.shared_ndarray import SharedNDArray
...
if __name__ == "__main__":
    ...
    with SharedMemoryManager() as shm_manager:
        ...
        # Create a queue with buffer size 10
        ...
        for i in range(5):
            data = {
                "image": np.ones((32, 32, 3), dtype=np.uint8) * i,
                "score": float(i)
            }
            queue.put(data)
            print(f"Put item {i}")
        ...
        for i in range(3):
            data = queue.get()
            print(f"Got item with score: {data['score']}")
        
        # Test getting all remaining items
        print("\nTesting get_all operation...")
        remaining = queue.get_all()
        print(f"Got {len(remaining['score'])} remaining items")
        
        # Test zero-copy view operations
        print("\nTesting zero-copy view operations...")
        view = queue.get_next_view()
        view["image"][:] = np.ones((32, 32, 3), dtype=np.uint8) * 100
        view["score"] = 100.0
        queue.put_next_view(view)
